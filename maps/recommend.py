...
def group_by_centroid(restaurants, centroids):
    ...
    for element in restaurants:
        initial_pairings += [[find_closest(restaurant_location(element), centroids), element]]
    return group_by_first(initial_pairings)
...
def find_centroid(cluster):
    ...
    for place in cluster:
        for latitude, longitude in [restaurant_location(place)]:
            lat += [latitude]
            lon += [longitude]
...
def rate_all(user, restaurants, feature_fns):
    ...
    for elem in restaurants:
        if elem in reviewed:
            d_ratings[restaurant_name(elem)] = user_rating(user, restaurant_name(elem))
        else:
            d_ratings[restaurant_name(elem)] = predictor(elem)
    return d_ratings
