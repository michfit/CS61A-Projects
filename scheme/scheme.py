...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) > len(formals):
            ...
            raise SchemeError('Not enough arguments')
        else:
            for elem in range(len(vals)):
                child.define(formals[elem], vals[elem])
...
def do_or_form(expressions, env):
    ...
    if eval_first is 0 or eval_first:
        return eval_first
    else:
...
def make_let_frame(bindings, env):
    ...
    def formals(bindings):
        if bindings == nil:
            return nil
        else:
            check_form(bindings.first, 2, 2)
            formals_lst = Pair(bindings.first.first, formals(bindings.second))
        return formals_lst

    check_formals(formals(bindings))  

    def list_expressions(bindings):
        if bindings == nil:
            return nil
        else:
            lst_expressions = Pair(scheme_eval(bindings.first.second.first, env), list_expressions(bindings.second))
        return lst_expressions

    child_let_frame = env.make_child_frame(formals(bindings), list_expressions(bindings))
    return child_let_frame
