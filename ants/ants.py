...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current = self.place
        transition = 0
        while current != hive and transition >= min_range and transition <= max_range :
            if current.bees:
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        ...
        transition = 0
        while current != hive:
            if transition >= LongThrower.min_range:
                if current.bees:
                    return random_or_none(current.bees)
            current = current.entrance
            transition += 1
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        current = self.place
        transition = 0
        while current != hive and transition >= ShortThrower.min_range and transition <= ShortThrower.max_range :
            if current.bees:
                return random_or_none(current.bees)
            else:
                current = current.entrance
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6A
        lst_of_bees = colony.active_bees
        for bee in lst_of_bees:
            print(bee)
            bee.reduce_armor(self.damage)
        # END Problem 6A
...
class ScubaThrower(ThrowerAnt):
    ...
    def reduce_armor(self, amount):
        if isinstance(self.place, Water):
            self.armor = 1
        else:
